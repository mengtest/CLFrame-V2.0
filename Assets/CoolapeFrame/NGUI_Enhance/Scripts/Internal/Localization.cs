...
public static class Localization
{
	...
	//add by chenbin
	static public void Load (string name, byte[] asset)
	{
		ByteReader reader = new ByteReader(asset);
		Set(name, reader.ReadDictionary());
	}
	...
	public static void append(string languageName, byte[] asset) {
		ByteReader reader = new ByteReader(asset);
		Dictionary<string, string> dictionary = reader.ReadDictionary ();
		if(mOldDictionary != null) {
			foreach (KeyValuePair<string, string> cell in mOldDictionary) {
				dictionary[cell.Key] = cell.Value;
			}
		}
		Set(languageName, dictionary);
	}
	#endregion
