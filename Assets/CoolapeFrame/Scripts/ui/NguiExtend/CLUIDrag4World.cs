...
//					dragProcType = 3;
				...
				} else if (Input.touchCount == 1) {	//一个手指
...
//							dragProcType = 1;
						...
						totalDelta1 = Vector2.zero;
						dragProcType = -1;
					} else if (Input.touches [0].phase == TouchPhase.Moved) { //滑动
						totalDelta1 += Input.touches [0].deltaPosition;
		...
		void proc2TouchsDrag (Vector2 iDelta)
		{
			...
			if (
				...
				(delta1.y > 0 && delta2.y > 0)) {		//两个手指向同一方向移动
				dragProcType = -1;
			} else if ((
...
//					float y3 = Mathf.Abs (dragDelta.y);
//					Vector2 delta = y1 > y2 ? (y3 > y1 ?  new Vector2 (0, y3) : new Vector2 (0, y1)) : (y3 > y2 ? new Vector2 (0, y3) : new Vector2 (0, y2));
...
//					float x3 = Mathf.Abs (dragDelta.x);
//					Vector2 delta = x1 > x2 ? (x3 > x1 ?  new Vector2 (0, -x3) : new Vector2 (0, -x1)) : (x3 > x2 ? new Vector2 (0, -x3) : new Vector2 (0, -x2));
